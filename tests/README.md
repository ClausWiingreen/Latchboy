...
Milestone 4 currently tracks CPU-adjacent Mooneye timing fixtures (`add_sp_e_timing.gb`, `call_cc_timing.gb`, `jp_cc_timing.gb`) in the manifest. If any of these entries are promoted to `required = true`, they become mandatory for the external validation pass/fail gate under `LATCHBOY_ROM_ROOT`.
