...
mod bench_impl {
    ...
    pub fn bench_register_alu_dispatch(c: &mut Criterion) {
    ...
    pub fn bench_cb_prefixed_bit_ops(c: &mut Criterion) {
        let cartridge = cartridge_with_program(
            &[
    ...
    pub fn bench_stack_and_control_flow(c: &mut Criterion) {
        let cartridge = cartridge_with_program(
            &[
...
criterion_group!(
    benches,
    bench_impl::bench_register_alu_dispatch,
    bench_impl::bench_cb_prefixed_bit_ops,
    bench_impl::bench_stack_and_control_flow
);
#[cfg(not(test))]
